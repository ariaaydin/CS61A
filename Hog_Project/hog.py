...
def roll_dice(num_rolls, dice=six_sided):
    ...
    while num_rolls > 0:
        result = dice()
        if result == 1:
            while num_rolls-1 > 0: # composition guidelines said this line isnt necessary but the code doesn't work without it, so its necesary and I'm leaving it in
                dice()
                num_rolls -= 1
            return 1
        else:
...
def swine_align(player_score, opponent_score):
    ...
    if player_score >0 and opponent_score>0:
        return gcd(player_score, opponent_score)
    else:
        return False
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        if who == 0:
            ...
            score0 += take_turn(rolls0, score1, dice)
            add_turn = extra_turn(score0, score1)
               
        else:
            rolls1= strategy1(score1, score0)
            score1 += take_turn(rolls1, score0, dice)
            add_turn = extra_turn(score1, score0)
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def higher(score0, score1):
        ...
        if difference>running_high:
            highestsofar = difference
            print(str(highestsofar) + ' point(s)! The most yet for Player ' + str(who))
        return announce_highest(who, score, highestsofar)
    return higher
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    """
    # BEGIN PROBLEM 9
    highest,i,returnval = 0,1,0
    while i <= 10:
        diceroll = make_averaged(roll_dice)
        theroll = diceroll(i, dice)
        if theroll > highest:
            highest = theroll
            returnval = i
        i += 1
...
def extra_turn_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    if extra_turn(score+take_turn(0,opponent_score), opponent_score):
        return 0
