...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 5
        name=target
        Frame.define(env, target, scheme_eval(expressions.rest.first, env)) 
        return name
        # END PROBLEM 5
        ...
        function=Pair(formals, body)
        Frame.define(env, name, do_lambda_form(function, env))
        return name
        # END PROBLEM 9
    else:
...
def make_let_frame(bindings, env):
    ...
    while bindings:
        ...
        values=Pair(scheme_eval(first.rest.first, env), values)
